...
4. Cliente pode converter `available` em crédito. O saldo é usado ao iniciar um checkout em Planos, não nas renovações automáticas. O checkout reserva o crédito e só o liquida no evento financeiro autoritativo; uma cobrança integralmente paga por crédito gera um `BillingEvent` interno. Crédito integral com cobrança externa já emitida exige conciliação antes de prosseguir e não consome saldo. Checkout anual pendente com valor diferente é cancelado antes da substituição; checkout já pago não consome crédito novo. Novos ciclos vencidos usam outra sessão apenas após confirmar uma nova cobrança pendente da mesma assinatura no Asaas, preservando reservas anteriores.
