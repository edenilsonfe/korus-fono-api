...
class AffiliatePolicy(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint("mode IN ('customer', 'partner')", name="ck_affiliate_policy_mode"),
        CheckConstraint(
    ...
    referral_discount_bps: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    commission_bps: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    customer_reward_monthly_cents: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    customer_reward_quarterly_cents: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    customer_reward_yearly_cents: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    ...
    payout_minimum_cents: Mapped[int] = mapped_column(Integer, nullable=False, default=10000)
    ...
    def snapshot(self, *, commission_bps: int | None = None) -> dict:
        return {
            ...
            "commissionBps": self.commission_bps if commission_bps is None else commission_bps,
...
class AffiliateParticipant(Base, TimestampMixin):
    ...
    customer_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    partner_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    customer_terms_version: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class AffiliateCode(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("mode IN ('customer', 'partner')", name="ck_affiliate_code_mode"),
        CheckConstraint("status IN ('active', 'revoked')", name="ck_affiliate_code_status"),
    )
...
class AffiliateReferral(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint("mode IN ('customer', 'partner')", name="ck_affiliate_referral_mode"),
        CheckConstraint(
    ...
    policy_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("affiliate_policies.id", ondelete="RESTRICT"), nullable=False
    )
    mode: Mapped[str] = mapped_column(String(16), nullable=False, index=True)
    ...
    benefit_expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class AffiliateReward(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "referral_id", "source_payment_id", "kind", name="uq_affiliate_reward_source"
        ),
        CheckConstraint(
    ...
    referral_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("affiliate_referrals.id", ondelete="RESTRICT"), nullable=False
    )
    source_payment_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
...
class AffiliateLedgerEntry(Base):
    ...
    reward_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("affiliate_rewards.id", ondelete="RESTRICT"), nullable=True
    )
    payout_request_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("affiliate_payout_requests.id", ondelete="RESTRICT"), nullable=True
    )
...
class AffiliateFiscalProfile(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("participant_id", "version", name="uq_affiliate_fiscal_version"),
        CheckConstraint("person_type IN ('pf', 'pj')", name="ck_affiliate_fiscal_person_type"),
        CheckConstraint(
    ...
    pix_validated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class AffiliatePayoutBatch(Base, TimestampMixin):
    ...
    approved_by_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("professionals.id", ondelete="RESTRICT"), nullable=True
    )
    approved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class AffiliatePayoutRequest(Base, TimestampMixin):
    ...
    provider_transfer_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
